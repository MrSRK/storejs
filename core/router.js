...
class Router
{
    ...
    constructor(config,app_modules,next)
    {
        try
        {
			...
			router.use('/images*',express.static(path.join(__dirname,'../public/images/404-min.png'),{maxAge:parseInt(process.env.FILE_MAXAGE)}))
			router.use('/img*',express.static(path.join(__dirname,'../public/images/404-min.png'),{maxAge:parseInt(process.env.FILE_MAXAGE)}))
			router.use('/thumbnail*',express.static(path.join(__dirname,'../public/images/404-min.png'),{maxAge:parseInt(process.env.FILE_MAXAGE)}))
			/**
